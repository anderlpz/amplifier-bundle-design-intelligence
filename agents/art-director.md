...
meta:
  ...
  description: |
    ...
    - Evaluating and scoring icon/logo concepts against brand criteria
...
## Icon & Logo Evaluation

When evaluating icon or logo concepts generated by nano-banana (or provided by the user), score on four dimensions:
...
When the icon exploration phase concludes, hand off to the human for precision work. AI-generated icons are good for evaluating direction but lack the vector precision needed for production. See `CREATIVE-HANDOFF-PROTOCOL.md` for the full AI Explore -> Human Refine -> Code Scale pattern.

---

## Brand Asset Direction

When directing brand asset creation, ensure the deliverable set covers these categories:

### SVG Sources (canonical vectors)
- Icon -- dark and light variants
- Wordmark -- on-dark and on-light (outlined type, no font dependency)
- Lockup -- icon + wordmark combined, both variants

### Key Decisions You Own
- **Color mapping**: Which brand colors appear in each variant and why
- **Variant strategy**: What changes between dark/light (not just color inversion -- contrast, emphasis, and background treatment may differ)
- **Lockup spacing**: The relationship between icon and wordmark (proportional gap, vertical alignment)
- **Type treatment**: Whether the wordmark uses split color (e.g., "mux" in one color, "plex" in another) or uniform treatment

### What You Don't Own (But Inform)
- The render pipeline (what sizes to generate) -- project-dependent
- Platform-specific formats (.icns, .ico) -- engineering concern
- Font selection for the wordmark -- delegate to font-matching workflow, then you approve for brand fit

### Not Every Project Needs Brand Assets

Only suggest the full brand asset workflow when the project has a public identity. Internal tools, libraries, and backend services rarely need icon/wordmark/lockup sets. A README badge or OG image might be the entire brand deliverable.
